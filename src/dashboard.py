...
def generate_pdf(user_data, ai_markdown):
    ...
    # 2. Cleanse user metadata strings to remove status emojis
    clean_username = re.sub(r'[^\x00-\x7F]+', '', str(user_data.get('username', 'N/A'))).strip()
    clean_uid = re.sub(r'[^\x00-\x7F]+', '', str(user_data.get('user_id', 'N/A'))).strip()
    clean_tier = re.sub(r'[^\x00-\x7F]+', '', str(user_data.get('risk_tier', 'N/A'))).strip()
    clean_impact = re.sub(r'[^\x00-\x7F]+', '', str(user_data.get('compliance_violations', 'N/A'))).strip()
    ...
    pdf.set_xy(120, 44)
    pdf.cell(40, 6, f"Risk Profile: {clean_tier} ({user_data.get('risk_score', 0)}/100)")
    
    pdf.set_xy(18, 52)
    pdf.cell(40, 6, f"Telemetry: {user_data.get('failed_logins', 0)} failed logins | {user_data.get('days_inactive', 0)} days inactive")
    pdf.set_xy(120, 52)
...
# 3. Data Layer Pipeline Resolution
try:
...
# 4. Compliance Rule Processor Engine (Moved AFTER data definition)
...
# Fallback column synchronization
if "regulatory_impact" not in df.columns:
    df["regulatory_impact"] = df["compliance_violations"]
...
with col4:
    non_compliant_count = len(df[df["compliance_violations"] != "Compliant"])
...
if st.button("🚀 Run AI Security & Compliance Audit"):
    ...
    with st.spinner(f"Generating AI Security Audit for {selected_user}..."):
        # Explicit data injection into the execution payload loop
        user_json_payload = user_data.to_json()
        
        prompt = (
            "You are an expert Cybersecurity Incident Responder and Global Compliance Auditor.\n"
            f"Analyze the provided user telemetry JSON data payload:\n{user_json_payload}\n\n"
            "You must explicitly evaluate the account anomalies against SOC 2, ISO 27001, GDPR, "
            "and India's Digital Personal Data Protection Act (DPDPA 2023). Output a professional "
            "Markdown executive threat summary, a framework violation checklist detailing exact clauses broken "
            "(e.g., DPDPA Sec. 8 regarding reasonable security safeguards to prevent data breaches), "
            "and immediate technical containment actions."
        )
